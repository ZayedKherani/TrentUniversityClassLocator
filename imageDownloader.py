...
with open('assetInfo.txt', 'w') as assetInfoFile:
    ...
    chromeService = Service(
        'C:/files and stuff/pathItems/chromedriver.exe', )
...
# write the photos to download to the assetUrlsInfo file
with open('assetUrlsInfo.txt', 'w') as assetUrlsInfoFile:
    # iterate through all photos to download
    for photoToDownloadIndex in range(len(photosToDownload)):
        # write the photo to download to the assetUrlsInfo file
        assetUrlsInfoFile.write(
...
print(f'{len(photosToDownload) * 3} images to download')
print(f'{round((len(completedPhotos)/(len(photosToDownload) * 3)), 4) * 100}% completed, {len(completedPhotos)} downloaded, downloading {str(None)}                                                                            ', end='\r', flush=True)
...
with open('assetInfoUrls.txt', 'w') as assetInfoUrlsFile:
    ...
    for photoToDownloadIndex in range(len(photosToDownload)):
        ...
        # print the number of photos to download and the number of photos downloaded
        print(f'{round((len(completedPhotos)/(len(photosToDownload) * 3)), 4) * 100}% completed, {len(completedPhotos)} downloaded, downloading {save_path}                                                                            ', end='\r', flush=True)
